...
void shiftCircular(Fila* f, Tipo indice){
    
    
    int aux = f->vetor[indice];
    int aux2; 

    while(indice != f->fim){
        indice = (indice+1 >= f->tam)? 0: indice;
        aux2 = f->vetor[indice+1];
        f->vetor[indice+1] = aux;
        aux = aux2;
        indice++;
    }
...
int findPosition(Fila* f, Tipo elemento){
    int begin =  f->inicio+1;
    int end = f->fim;

    while(begin != end){
        if(f->vetor[begin] > elemento){
            printf("Posicao: %d\n", begin);
            return begin;
        }else{
            if(begin+1 >= f->tam){
...
Boolean fila_inserir(Fila* f, Tipo elemento){
    ...
    if(f->vetor[f->inicio] > elemento){
        shiftCircular(f, f->inicio);
        f->vetor[f->inicio] = elemento;
        f->fim++;
    }else
    if(f->vetor[f->fim-1] < elemento){
        f->vetor[f->fim] = elemento;
        f->fim++;
    }else{
        int indice = findPosition(f, elemento);
        shiftCircular(f, indice);
        f->vetor[indice] = elemento;
        f->fim++;
    }
